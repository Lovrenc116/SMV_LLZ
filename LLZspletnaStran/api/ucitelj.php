...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        // Če je podan ID učitelja, vrni njegove predmete
        if (isset($_GET['id']) && $_GET['id']) {
            $uciteljId = (int)$_GET['id'];
            $stmt = $pdo->prepare('SELECT predmet_id FROM ucitelji_predmeti WHERE ucitelj_id = ?');
        ...
        // List all teachers with aggregated subjects
        $sql = "
            SELECT u.id, u.ime, u.priimek, u.email,
            ...
            GROUP BY u.id, u.ime, u.priimek, u.email
            ORDER BY u.id
        ";
    ...
    case 'POST':
        ...
        try {
            ...
            if ($id) {
                if ($geslo !== '') {
                    $stmt = $pdo->prepare('UPDATE ucitelji SET ime = ?, priimek = ?, email = ?, geslo = ? WHERE id = ?');
                    $stmt->execute([$ime, $priimek, $email, $geslo, $id]);
                } else {
                    $stmt = $pdo->prepare('UPDATE ucitelji SET ime = ?, priimek = ?, email = ? WHERE id = ?');
                ...
                $stmt = $pdo->prepare('INSERT INTO ucitelji (ime, priimek, email, geslo) VALUES (?, ?, ?, ?)');
