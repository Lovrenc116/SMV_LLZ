...
<body>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      async function osveziNaloge() {
        try {
          const response = await fetch('get_naloge.php');
          const naloge = await response.json();
          ...
          naloge.forEach(naloga => {
            ...
            row.innerHTML = `
              <td>${naloga.id}</td>
              <td>${naloga.predmet}</td>
              <td>${naloga.ucenec}</td>
              <td>${naloga.naziv}</td>
              <td><a href="${naloga.datoteka}" class="download-link" download><i class="fas fa-download"></i> ${naloga.datoteka.split('/').pop()}</a></td>
              <td>${naloga.datum_oddaje}</td>
            `;
