      ...
      <div id="nalogeSection" class="mt-6 hidden">
        ...
        <form id="nalogaForm" enctype="multipart/form-data" class="mt-6 space-y-4">
          <div>
            ...
            <input type="text" id="naslovNaloge" name="naslov" required class="mt-1 w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
          </div>
          <div>
  ...
  <script>
    ...
    async function naloziPredmete() {
      try {
        ...
        select.innerHTML = '<option value="">Izberi...</option>' + predmeti.map(p => `<option value="${p.id}">${p.naziv_predmeta}</option>`).join('');
      } catch (error) {
